...
도메인 주도 설계를 적용하려면 먼저 비즈니스 하위 도메인을 정의해야 합니다. 그 다음, 관계형 데이터베이스에 저장할 애그리거트/엔티티를 정의할 수 있습니다.
...
도메인 주도 설계에서 애그리거트 루트는 "전체 애그리거트에 대한 게이트웨이 역할을 하는 엔티티로, 애그리거트의 무결성과 일관성을 유지하는 역할"을 합니다. 간단히 말해, 서비스에서 가장 중요하고 관련성 있는 주요 엔티티를 찾는 것입니다.
...
이러한 요구사항을 바탕으로 `Customer`, `Order`, `Product`를 애그리거트 루트로 그룹화할 수 있습니다.

#### 휴리스틱 1: Spring Repository는 애그리거트 루트 엔티티에 대해서만 정의되어야 합니다.
...
#### 휴리스틱 2: 애그리거트 내에서는 `@OneToOne` 또는 일부 `@ManyToOne` 대신 Value 객체 사용을 고려하세요.
...
#### 휴리스틱 3: 애그리거트 루트 간의 관계 매핑은 피하고, 외래 키를 그대로 매핑하세요.
...
#### 휴리스틱 4: 다른 애그리거트 루트와의 관계 매핑은 관계가 유한할 때만 허용하세요.
...
#### 휴리스틱 5: 항상 단방향 관계로 시작하세요. 필요가 생기고 관계가 애그리거트 루트를 넘지 않을 때만 양방향 관계로 변환하세요.
...
#### 휴리스틱 6: 애그리거트 루트에서 모든 자식 관계를 지연 로딩으로 시작하세요. 관련 엔티티가 항상 필요한 경우에만 즉시 로딩으로 변환하세요.
...
#### 휴리스틱 7: 다른 애그리거트 루트와의 관계가 있다면 절대 즉시 로딩을 사용하지 마세요.
...
#### 휴리스틱 8: 애그리거트 루트가 아닌 모든 엔티티에 대해 연쇄 작업을 설정하세요. 연관된 애그리거트 엔티티에 대해서는 연쇄 작업을 설정하지 마세요.
...
public class Order {
    ...
    private Long customerId; // 다른 애그리거트 루트에 대해서는 cascade 설정 없음
    // other fields, getters and setters
}
```
        '></mark-down>
...
1. Spring Repository는 애그리거트 루트 엔티티에 대해서만 정의되어야 합니다.
2. 애그리거트 내에서는 `@OneToOne` 또는 일부 `@ManyToOne` 대신 Value 객체 사용을 고려하세요.
3. 애그리거트 루트 간의 관계 매핑은 피하고, 외래 키를 그대로 매핑하세요.
4. 다른 애그리거트 루트와의 관계 매핑은 관계가 유한할 때만 허용하세요.
5. 항상 단방향 관계로 시작하세요. 필요가 생기고 관계가 애그리거트 루트를 넘지 않을 때만 양방향 관계로 변환하세요.
6. 애그리거트 루트에서 모든 자식 관계를 지연 로딩으로 시작하세요. 관련 엔티티가 항상 필요한 경우에만 즉시 로딩으로 변환하세요.
7. 다른 애그리거트 루트와의 관계가 있다면 절대 즉시 로딩을 사용하지 마세요.
8. 애그리거트 루트가 아닌 모든 엔티티에 대해 연쇄 작업을 설정하세요. 연관된 애그리거트 엔티티에 대해서는 연쇄 작업을 설정하지 마세요.
